...
Original file is located at
    https://colab.research.google.com/drive/1-JqrUZ_K71DCD3q_3lqkJCqkWP-_7DOU
"""
...
nncf_config = NNCFConfig.from_dict({
    "input_info": {
        "sample_size": [1, 6]  # Adjust to the correct input shape for your model
    },
    "compression": {
...
class CalibrationDataset(Dataset):
    def __init__(self, num_samples, batch_size=1): # Use __init__ for the constructor
        self.num_samples = num_samples
        self.batch_size = batch_size

    def __len__(self): # Use __len__
        return self.num_samples

    def __getitem__(self, item): # Use __getitem__
        # Generate a tensor with the correct shape [1, 7]
        input_ids = torch.randint(0, 100, (1, 6), dtype=torch.int32)  # Ensure the shape is [1, 7]
        attention_mask = torch.ones((1, 6), dtype=torch.int32)        # Ensure the shape is [1, 7]
    ...
    def get_length(self):
        return self.__len__() # Call __len__
...
# Define the output directory - make sure this path exists
output_dir = "/content/blenderbot_openvino_ir" # Example path, change as needed
...
model_dir = "blenderbot_openvino_ir"
chat_pipeline = pipeline("text2text-generation", model=model_dir, tokenizer="facebook/blenderbot-90M")
